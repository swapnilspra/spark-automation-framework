...
class Job(ETLJob):
    ...
    def extract(self,catalog:Dict[str,Any]) -> Dict[str,pyspark.sql.DataFrame]:
        ...
        #df_temp = df_temp.filter(df_temp["`Asof Date0`"].isNotNull())
        ...
        df_transformed = df_transformed.select("*",F.explode(F.col("charc_value")).alias("charc_dict"))\
            .select(
                F.col("Acct No1").alias("acct"),
                F.col("Asof Date0").alias("date"),
                "charc_dict.charnm",
                "charc_dict.fund_val",
                "charc_dict.bmk_val"
            ).drop("charc_value")
...
def map_fields(row) -> List[Dict[str,Any]]:
    ...
    return_rows:List[Dict[str,Any]] = [
        {
            "charnm":"Total Market Value",
            "fund_val": row['Mkt Val Total'],
            "bmk_val": None
        },
        {
            "charnm":"Quarterly Contrib",
            "fund_val": row['Qtrly contrib'],
            "bmk_val": None
        },
        {
            "charnm":"Estimated Income",
            "fund_val": row['Estimated Income'],
            "bmk_val": None
        },
        {
            "charnm":"Estimated YTM",
            "fund_val": row['Estimated YTM'],
            "bmk_val": row['Benchmark Estimated YTM']
        },
        {
            "charnm":"Modified Duration (yrs)",
            "fund_val": row['Duration6'],
            "bmk_val": row['Duration15']
        },
        {
            "charnm":"Weighted Avg. Maturity (yrs)",
            "fund_val": row['Avg Maturity'],
            "bmk_val": None
        },
        {
            "charnm":"Avg. Quality",
            "fund_val": str(row['Avg Qlty']),
            "bmk_val": str(row['Bogie Avg Qlty'])
        },
        {
            "charnm":"Average Market Coupon (%)",
            "fund_val": row['Avg Coupon'],
            "bmk_val": None
        },
        {
            "charnm":"Current Yield (%)",
            "fund_val": row['Current Yield'],
            "bmk_val": row['Benchmark - Current Yield(%)']
        },
        {
            "charnm":"Duration - PIMCO (yrs)",
            "fund_val": None,
            "bmk_val": row['Benchmark Duration - PIMCO(yrs)']
        }
    ]
    return return_rows