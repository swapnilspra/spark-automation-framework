from pyspark.sql.types import ArrayType, StringType, MapType, StructField, StructType, FloatType
from typing import Dict, List, Any
import math
import logging
import pyspark.sql
import common.utils
import pyspark.sql.functions as F
from common.etl_job import ETLJob # must be imported after spark has been set up
from datetime import datetime
class Job(ETLJob):
    target_table = "fi_country_allocation"
    business_key = ["fund_compst_key","day_key","fi_crty_key"]
    primary_key = {"fi_crty_allocn_key":"int"}
    sources:Dict[str,Dict[str,Any]] = {
        "infile": {
            "type": "file",
            "source": "pimco_country"
        },
        "parent": {
            "type": "table",
            "source": "fi_country"
        },
        "acctref": {
            "type": "dimension",
            "source": "pimco_account_reference"
        },
        "cal": {
            "type": "table",
            "source": "calendar"
        },
        "fundcomp": {
            "type": "table",
            "source": "fund_composite"
        }
    }
    joins:List[Dict[str,Any]] = [
        {
            "source": "infile"
        },
        {
            "source": "acctref",
            "conditions": [
                F.upper(F.col("infile.`Acct No`")) == F.col("acctref.PMC_ACCT_NBR")
            ]
        },
        {
            "source": "fundcomp",
            "conditions": [
                F.upper(F.col("acctref.`FUND_COMPST_NM`")) == F.upper(F.col("fundcomp.compst_nm"))
            ]
        },
        {
            "source": "cal",
            "conditions": [
                F.date_trunc('day',(F.to_timestamp(F.col("infile.`Asof Date`"), "MM/dd/yyyy"))) == F.to_date(F.col("cal.cal_day"))
            ]
        },
        {
            "source": "parent",
            "conditions": [
                F.upper(F.col("infile.`Country Description`")) == F.upper(F.col("parent.crty"))
            ]
        }
    ]    
    target_mappings:List[Dict[str,Any]] = [
        {"source": F.col("fundcomp.fund_compst_key"), "target": "fund_compst_key"},
        {"source": F.col("cal.day_key"), "target": "day_key"},
        {"source": F.col("parent.fi_crty_key"), "target": "fi_crty_key"},
        {"source": F.col("infile.ACCT_CURREXP")/100, "target": "fund_curr_expsr"},
        {"source": F.col("infile.ACCT_EM_SPREAD"), "target": "fund_ext_em_sprd"},        
        {"source": F.col("infile.BM_EM_SPREAD"), "target": "bmk_ext_em_sprd"},
        {"source": F.col("infile.BM_CURREXP"), "target": "bmk_curr_expsr"},
        {"source": F.col("infile.ACCT_DUR_YRS7"), "target": "fund_ext__dur_yr"},
        {"source": F.col("infile.ACCT_DUR_YRS9"), "target": "fund_loc_dur_yr"},
        {"source": F.col("infile.BM_DUR_YRS11"), "target": "bmk_ext_dur_yr"},
        {"source": F.col("infile.BM_DUR_YRS13"), "target": "bmk_loc_dur_yr"},
        {"source": F.lit("Y"), "target": "curr_row_flg"}
    ]

