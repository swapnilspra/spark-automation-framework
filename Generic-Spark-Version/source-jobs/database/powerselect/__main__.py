...
# create daily workflow to run "shareholder" powerselect jobs
daily_wf = WorkflowSchedule(f"powerselect_daily_{env}",
    source_jobs=[job for (name,job) in jobs.items() if 'shareholder' in name],
    schedule_expr="0 3 * * ? *",
    enabled=(env in ['qa','datamart']))

# create monthly workflow to run other powerselect jobs
monthly_wf = WorkflowSchedule(f"powerselect_monthly_{env}",
    source_jobs=[job for (name,job) in jobs.items() if not 'shareholder' in name],
    schedule_expr="0 8 * * ? *",
    enabled=(env in ['qa','datamart']))